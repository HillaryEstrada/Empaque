...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['tablas']) || !isset($_POST['redirect_tabla'])) {
        ?>
        <script>
            Swal.fire({
                icon: 'error',
    ...
    $estado_redirect = isset($_POST['estado_redirect']) ? $_POST['estado_redirect'] : '1';
    ...
    try {
        foreach ($tablas_array as $tabla_info) {
            ...
            $resultado = Modelo::desactivarElementoModelo($pk_value, $tabla, $pk_name);
        ...
        if (!$error && !in_array('error', $resultados)) {
            ...
            <script>
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Ocurrió un error al desactivar uno o más registros.'
                });
            </script>
            <?php
        }
